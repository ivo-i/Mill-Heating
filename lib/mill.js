...
const { dDebug, dWarn, dError } = require('./util');
...
class Mill {
  ...
  async login(username, password) {
    ...
    try {
      ...
      dDebug('Logged in', this.user);

      return data;
    } catch (error) {
      if (error.response) {
        dError(error.response.data);
        dError(error.response.status);
        dError(error.response.headers);
      } else if (error.request) {
        dError(error.request);
      } else {
        dError('Error', error.message);
      }
      if (error.response && error.response.data.error.type === 'LogInError') {
        dError(error.response.data.error.message);
      }
    }
  }
