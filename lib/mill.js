...
class Mill {
  ...
  async checkMigrationStatus(email) {
    ...
    const endpoint = `${this.endpoint}/cloud-migration/migration-status?loginType=email&login=${email}`;
  ...
  async migrateCustomer(email, password) {
    const headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/json',
    };
    ...
    const json = await axios.post(endpoint, { email, password }, { headers });
