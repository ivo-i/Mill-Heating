...
class Mill {
  ...
  async login(username, password) {
    ...
    if (json.error) {
      ...
      this.auth = {
        ...
        tokenExpire: new Date().getTime() + 10 * 60 * 1000,
      };
  ...
  async updateAccessToken() {
    ...
    const json = await axios({
      ...
      if (error.status === 401) {
        ...
        return await this.login(this.user.login, this.user.password);
      }
      console.log(error.error, error.status, error.data);
  ...
  async request(command, body) {
    ...
    if (!this.auth.token && !this.auth.refreshToken) {
      ...
      await this.login(this.user.login, this.user.password);
