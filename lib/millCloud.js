...
class MillCloud extends millAPI {
	...
	async request(command, body = null, requestMethod = 'get') {
		...
		try {
			...
			if (response.data.rooms?.roomControlSource !== 'open_api' || response.data?.controlSource !== 'open_api') {
	...
	async updateRoomControlSource(roomId, controlSource) {
		const body = {
	...
	async updateControlSource(deviceId, controlSource) {
		const body = {
			deviceIds: [deviceId],
			controlSource: controlSource,
		};
	...
	async updateControlSources(response) {
		try {
			// Først prøver vi å oppdatere roomControlSource hvis den er tilgjengelig og ikke allerede 'open_api'
			if (response.data.rooms?.roomControlSource !== 'open_api') {
				this.app.dDebug(`[${this.app.homey.manifest.name.en}] Room control source is mobile_api. Trying to update to open_api...`);
				await this.updateRoomControlSource(response.data.rooms.id, 'open_api');
				this.app.dDebug(`[${this.app.homey.manifest.name.en}] Room control source set to open_api`);
			}
		} catch (err) {
			this.app.dError(`[${this.app.homey.manifest.name.en}] Error setting room control source to open_api. Trying to update device control source instead.`, err.message);
			// Hvis det feiler, prøver vi å oppdatere controlSource for enheten
			try {
				if (response.data?.controlSource !== 'open_api') {
					await this.updateControlSource(response.data.deviceId, 'open_api');
					this.app.dDebug(`[${this.app.homey.manifest.name.en}] Device control source set to open_api`);
				}
			} catch (err) {
				this.app.dError(`[${this.app.homey.manifest.name.en}] Error setting device control source to open_api. Giving up...`, err.message);
			}
		}
	}
