...
class MillLocal extends millAPI {
    ...
    async autoScan(ip) {
        ...
        // Vi validerer IP-adressen som er oppgitt. Vi henter ut de tre første oktettene og sjekker at de er gyldige.
        // Vi sjekker også at det er et tall mellom 0 og 255.
        ...
        // Vi setter sammen de tre første oktettene til en IP-adresse.
        const ipBase = `${ipParts[0]}.${ipParts[1]}.${ipParts[2]}`;

        // Vi lager en liste over de IP-adressene vi skal sjekke.
        const ipList = [];
        for (let i = 1; i < 255; i++) {
            ipList.push(`${ipBase}.${i}`);
        }

        // Vi lager en liste over alle IP-adressene som svarer.
        const responseList = [];
        for (const ip of ipList) {
            // Vi bruker try/catch for å fange opp feil som oppstår når vi prøver å koble til en IP-adresse.
            try {
                ...
                if (response.status === 200 && response.data.status === 'ok') {
                    ...
                    responseList.push(ip);
                }
            } catch (error) {
                // Vi ignorerer feil som oppstår når vi prøver å koble til en IP-adresse.
                if (error.response && error.response.status === 404) {
                    console.log(`Error while checking IP-address ${ip}`);
                }
            }
        }
    }
