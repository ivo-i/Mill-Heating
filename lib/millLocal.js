...
class MillLocal extends millAPI {
    ...
    async autoScan(ip, callback) {
        ...
        for (let i = 2; i < 255; i++) {
            ...
            try {
                callback({ ip: currentIp, status: 'checking' });
                const response = await axios.get(`http://${currentIp}/status`, { timeout: 500 });
                if (response.data && response.data.status === 'ok') {
                    response.data.ip_address = currentIp;
                    responseList.push(response.data);
                    callback({ ip: currentIp, status: 'found', data: response.data });
                }
            } catch (error) {
                callback({ ip: currentIp, status: 'not found' });
            }
        }

        if (responseList.length > 0) {
            callback({ status: 'done', message: 'Scanning complete, devices found.' });
            return { success: true, data: responseList };
        } else {
            callback({ status: 'done', message: 'Scanning complete, no devices found.' });
            return { success: false, message: 'No Mill devices found' };
        }
    }
}
